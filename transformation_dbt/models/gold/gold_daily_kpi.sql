...
silver_loan_defaults AS (
  SELECT
    ...
    sl.loan_id,
    COUNT_IF(
      sp.status IN ('failed', 'missed')
      AND sp.payment_dt BETWEEN sl.funded_at AND (sl.funded_at + INTERVAL 90 DAY)
    ) > 0 AS has_defaults
  FROM
    silver_loans AS sl
    LEFT JOIN {{ ref('silver_payments')}} AS sp ON sl.loan_id = sp.loan_id
  GROUP BY
    sl.funded_at,
    sl.loan_id
)
...
FROM
  ...
  LEFT JOIN silver_loan_defaults_percentage sldp ON slg.funded_at = sldp.funded_at
ORDER BY
  slg.funded_at
